...
class Framework_of_Oz_Metabox{
	...
	// $groupTree (ARR) The list of current ID's for nested groups, [x][y]
	//===============================================
	private function display_field($field, $key, $parents = array(), $groupID = 0, $groupTree = array()){
		global $oz;
					...
					echo "<div class='oz-group-pair'><div class='oz-group-handle' data-original-label='$label'>
						...
						<div class='oz-group-panel $state'>";
							...
							foreach($field['fields'] as $key=>$childField)
								$this->display_field($childField, $key, $parents, $i, $groupTree);
					echo '</div></div>';
				endfor;
			//- - - - - - - - - - - - - - - - - - - - - - - -
			// Close it
		...
		//- - - - - - - - - - - - - - - - - - - - - - - -
		if(count($parents)){
			if(count($groupTree) < 2){
				$id .= '[' . $groupID . ']';
				$value = isset($value[$groupID]) ? $value[$groupID] : '';
			} else {
				$tmpGroupTree = array_slice($groupTree, 1);
				$groupStr = '';
				foreach($tmpGroupTree as $groupNode)
					$groupStr .= "[$groupNode]";
				$id .= $groupStr . '[' . $groupID . ']';
				eval('$value = isset($value'.$groupStr.'['.$groupID.']) ? $value'.$groupStr.'['.$groupID.'] : "";');
			}
		}
		$name 	= $id;
				...
				echo '<div class="oz-field-repeat-wrap">';
					...
					switch($oz->def($type, 'text')){
						...
						case 'select':
							...
							echo "<select id='$id' data-original-id='$originalID' class='$class' name='$name' $atts>";
						...
						case 'file':
							$value = esc_attr($value);
							echo "<input id='$id' data-original-id='$originalID' class='$class width-three-quarters' name='$name' type='text' value='$value' $atts>";
						...
						//===============================================
						case 'submit':
							echo "<input id='$id' data-original-id='$originalID' class='$class button-primary' type='$type' value='$button' $atts>";							
						break;
